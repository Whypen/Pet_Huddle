...
begin
  ...
  -- display_name only — never use social_id in notification copy
...
begin
  ...
  if v_alert.type_norm = 'stray' then
    ...
    v_body := 'A pet is lost near ' || v_district || '. Tap to help bring them home 🐾';
  end if;
  ...
  for rec in
    ...
    where not public.is_user_blocked(p.id, v_alert.creator_id)
      and (
        (
          nullif(btrim(p.location_country), '') is not null
          and v_alert.location_country is not null
          and lower(nullif(btrim(p.location_country), '')) = lower(v_alert.location_country)
        )
        or (
          coalesce(p.location, p.location_geog) is not null
          and v_alert.source_geog is not null
          and st_dwithin(coalesce(p.location, p.location_geog), v_alert.source_geog, 150000)
        )
      )
  loop
    if not exists (
...
CREATE OR REPLACE FUNCTION "public"."is_social_id_taken"("p_social_id" "text") RETURNS boolean
  ...
  select exists (
    select 1
    from public.profiles
    where lower(social_id) = lower(trim(p_social_id))
  );
$$;
...
COMMENT ON COLUMN "public"."profiles"."care_circle" IS 'List of user IDs who have permanent emergency access to this location.';
...
CREATE INDEX "notif_agg_windows_lookup" ON "public"."notification_aggregation_windows" USING "btree" ("owner_user_id", "subject_id", "kind");
