...
Thresholds carried over from earlier drafts; worth re-checking against real 90-day distributions once the view is live, since point values per action changed since these were set (e.g. a single completed carer booking is +10, a fifth of the way to "Active"). Tuning the threshold numbers later is a config change, not a re-architecture.
