...
-- 1. Seeded SF/HK users must carry real geog, not only lat/lng display columns.
-- 2. If viewer has a geog anchor, discovery is radius-scoped only.
-- 3. City/country fallback applies only when there is no viewer geog anchor.

update public.profiles
set
  location = st_setsrid(st_makepoint(coalesce(longitude, last_lng), coalesce(latitude, last_lat)), 4326)::geography,
  location_geog = st_setsrid(st_makepoint(coalesce(longitude, last_lng), coalesce(latitude, last_lat)), 4326)::geography,
  last_active_at = coalesce(last_active_at, now()),
  updated_at = now()
where (social_id like 'sf_test_%' or social_id like 'hk_test_%')
  and coalesce(latitude, last_lat) is not null
  and coalesce(longitude, last_lng) is not null;
    ...
    select
      ...
      coalesce(
        ...
        case
          ...
          then st_setsrid(
            st_makepoint(coalesce(p.longitude, p.last_lng), coalesce(p.latitude, p.last_lat)),
            4326
          )::geography
          else null::geography
        end
