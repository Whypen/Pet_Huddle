create or replace function public.update_group_chat_metadata(
  ...
  p_update_location boolean default false,
  p_update_pet_focus boolean default false
)
returns table(
  id uuid,
  name text,
...
begin
  ...
  set
    ...
    location_label = case
      when p_update_location then nullif(btrim(coalesce(p_location_label, '')), '')
      else c.location_label
    end,
    pet_focus = case
      when p_update_pet_focus then coalesce(p_pet_focus, array[]::text[])
      else c.pet_focus
    end,
    updated_at = now()
  where c.id = p_chat_id
...
grant execute on function public.update_group_chat_metadata(uuid, text, text, text, text, text[], boolean, boolean, boolean, boolean, boolean) to authenticated;
