-- /admin/safety disputes demo/evidence + canonical money math fix
-- Safe admin decision layer only, no Stripe execution.

begin;

-- Replace with waive-platform-fee aware admin decision function.
...
create or replace function public.admin_apply_dispute_decision(
  ...
  p_waive_platform_fee boolean default false
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_platform_fee_retained numeric := 0;
...
begin
  ...
  v_service_rate := greatest(v_total_paid - v_platform_fee, 0);

  if v_action = 'release_full' then
    v_status := 'resolved_release_full';
    v_refund := 0;
    v_provider := v_service_rate;
    v_platform_fee_retained := v_platform_fee;
    ...
    v_max_refund := case when coalesce(p_waive_platform_fee, false) then v_total_paid else v_service_rate end;
    v_refund := greatest(least(p_customer_refund_amount, v_max_refund), 0);
    v_platform_fee_retained := case when coalesce(p_waive_platform_fee, false) then 0 else v_platform_fee end;
    v_provider := greatest(v_total_paid - v_platform_fee_retained - v_refund, 0);
    v_status := 'resolved_partial_refund';
    v_audit_action := 'disputes_partial_refund';
  else
    v_status := 'resolved_refund_full';
    v_platform_fee_retained := case when coalesce(p_waive_platform_fee, false) then 0 else v_platform_fee end;
    v_refund := case when coalesce(p_waive_platform_fee, false) then v_total_paid else v_service_rate end;
    v_provider := 0;
  ...
  v_money := jsonb_build_object(
    ...
    'waive_platform_fee', coalesce(p_waive_platform_fee, false),
    'platform_fee_retained_amount', v_platform_fee_retained,
...
revoke all on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) from public;
revoke all on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) from anon;
revoke all on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) from authenticated;
revoke all on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) from service_role;
grant execute on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) to authenticated;
grant execute on function public.admin_apply_dispute_decision(uuid, text, text, numeric, boolean) to service_role;

-- Canonical queue math: total paid, platform fee, provider receives (service rate), customer refunded.
...
-- Backfill existing demo fixture rows to real links + canonical fixture values.
...
set
  quote_card = jsonb_set(
    ...
    '{platform_fee_amount}', '"10.00"'::jsonb, true
  ),
  updated_at = now()
...
set
  quote_card = jsonb_set(
    ...
    '{platform_fee_amount}', '"20.00"'::jsonb, true
  ),
  updated_at = now()
...
set
  ...
  decision_payload = jsonb_set(
    ...
    jsonb_build_object(
      ...
      'platform_fee_retained_amount', 10.00,
      'provider_receives_amount', 100.00,
...
set
  ...
  decision_payload = jsonb_set(
    ...
    jsonb_build_object(
      ...
      'waive_platform_fee', false,
      'platform_fee_retained_amount', 20.00,
      'provider_receives_amount', 200.00,
      'customer_refund_amount', 0
    ),
    true
  ),
