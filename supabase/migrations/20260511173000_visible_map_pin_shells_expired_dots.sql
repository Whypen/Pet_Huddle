...
as $$
  ...
  alert_shells as (
    select
      ...
      case
        when b.created_at + make_interval(hours => greatest(1, least(72, coalesce(b.duration_hours, 24)))) > now()
          then lower(coalesce(b.type, 'alert'))
        else 'expired_' || lower(coalesce(b.type, 'alert'))
      end as pin_type,
    ...
    where b.creator_id is not null
      and not exists (
        select 1 from blocked x where x.user_id = b.creator_id
      )
  ...
  user_shells as (
    ...
    where p.viewer_id is not null
      ...
      and not exists (
        select 1 from blocked x where x.user_id = pr.id
      )
