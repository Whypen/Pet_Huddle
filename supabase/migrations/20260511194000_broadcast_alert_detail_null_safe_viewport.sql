...
as $$
  select
    ...
    b.created_at,
    (b.created_at + make_interval(hours => greatest(1, least(72, coalesce(b.duration_hours, 24))))) as expires_at,
    b.duration_hours,
    greatest(1, floor((coalesce(b.range_km, 5) * 1000.0))::int) as range_meters,
    b.range_km,
    b.creator_id,
    b.thread_id,
    ...
    case
      when now() <= (b.created_at + make_interval(hours => greatest(1, least(72, coalesce(b.duration_hours, 24))))) then 'active'
      when now() <= (b.created_at + make_interval(hours => greatest(1, least(72, coalesce(b.duration_hours, 24)))) + interval '7 days') then 'expired_dot'
      else 'hidden'
    end as marker_state
  ...
  where b.id = p_alert_id
    ...
    and (
      ...
      or now() <= (b.created_at + make_interval(hours => greatest(1, least(72, coalesce(b.duration_hours, 24)))) + interval '7 days')
    )
    and (
      auth.uid() is null
    ...
    and (
      auth.uid() is null
      or not public.is_user_blocked(auth.uid(), b.creator_id)
    )
  limit 1;
$$;

revoke all on function public.get_broadcast_alert_by_id(uuid) from public, anon;
