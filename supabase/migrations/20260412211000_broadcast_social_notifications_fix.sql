-- Fix broadcast "On Social" FK mismatch and restore broadcast notification fan-out.

alter table if exists public.threads
  drop constraint if exists threads_map_id_fkey;

alter table if exists public.threads
  add constraint threads_map_id_fkey
  foreign key (map_id)
  references public.broadcast_alerts(id)
  on delete set null
  not valid;
...
begin
  ...
  if has_href then
    cols := cols || 'href';
    exprs := exprs || $q$'/map?alert=' || $5::text$q$;
  end if;

  if has_data then
    cols := cols || 'data';
    exprs := exprs || $q$jsonb_build_object('alert_id', $5, 'alert_type', $4, 'thread_id', $6)$q$;
  end if;
