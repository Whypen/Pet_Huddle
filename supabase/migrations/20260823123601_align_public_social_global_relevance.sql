-- Logged-out Social follows the native relevance contract: location ranks the
-- feed but never gates it. With no authenticated viewer scope, Vercel's coarse
-- country is the only locality signal. Same-country posts receive the native
-- country weight/Latest lift; global posts remain eligible everywhere.
...
as $$
  with candidates as materialized (
    select
      ...
      case
        when nullif(btrim(coalesce(p_cursor->>'country', '')), '') is not null
          and public.normalize_country_key(t.post_country) = public.normalize_country_key(p_cursor->>'country')
        then 3
        else 9
      end as local_scope_priority
  ...
  scored as (
    select
      ...
      (
        ...
        / power(greatest(extract(epoch from (now() - c.created_at)) / 3600.0, 0::numeric) + 2::numeric, 1.5::numeric)
        * case c.local_scope_priority when 3 then 0.40::numeric else 0.15::numeric end
      )::numeric as computed_score,
      (
        extract(epoch from c.created_at)
        + case c.local_scope_priority when 3 then 21600::numeric else 0::numeric end
      )::numeric as latest_score
    from candidates c
    left join lateral (
  ...
  order by
    ...
    case when lower(coalesce(p_cursor->>'sort', 'latest')) <> 'trending' then s.latest_score end desc nulls last,
    s.created_at desc,
    s.id desc
