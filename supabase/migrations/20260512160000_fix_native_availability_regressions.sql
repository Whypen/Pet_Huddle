...
begin
  ...
  where bucket_id = 'identity_verification'
    and name in (
      ...
      where verification_status in ('verified'::public.verification_status_enum, 'unverified'::public.verification_status_enum)
...
begin
  ...
  pet_data as (
    select
      pe.owner_id,
      array_remove(array_agg(distinct pe.species), null) as species,
      max(
        case
          when pe.weight is null then null
          when pe.weight_unit = 'lb' then pe.weight * 0.453592
          else pe.weight
        end
      ) as max_weight_kg
    from public.pets pe
    where coalesce(pe.is_active, true) = true
    group by pe.owner_id
  )
  select
    p.id,
    p.display_name,
    p.avatar_url,
    case
      when jsonb_typeof(p.photos) = 'array' then array(select jsonb_array_elements_text(p.photos))
      else '{}'::text[]
    end as photos,
    ...
    case
      ...
      when pd.max_weight_kg <= 9 then 'Small'
      when pd.max_weight_kg <= 22 then 'Medium'
      else 'Large'
    end as pet_size,
    p.pet_experience,
    p.experience_years as pet_experience_years,
    p.experience_years,
    p.languages,
    p.social_album,
...
from auth.users au
where au.id = p.id
  and au.email ~* '^testaccount(0[2468]|[123][02468]|40)@huddle\\.test$'
  and exists (
    select 1
