...
-- Money locks are booking scoped. A conversation room may contain many bookings,
-- so every payout mutation now matches only service_chats.id.
create or replace function public.claim_service_payout_release(p_chat_id uuid, p_lock_token text)
returns jsonb language plpgsql security definer set search_path = public as $function$
declare v_row public.service_chats%rowtype;
begin
  update public.service_chats
  set payout_release_lock_token = p_lock_token, payout_release_locked_at = now(),
      payout_release_attempted_at = now(), payout_release_failed_at = null,
      payout_release_failure_reason = null
  where id = p_chat_id
    and status in ('completed', 'cancelled')
    and payout_released_at is null and stripe_transfer_id is null
    and manual_recovery_required_at is null
    and (payout_release_lock_token is null or payout_release_locked_at < now() - interval '10 minutes')
...
create or replace function public.unlock_service_payout_release(p_chat_id uuid, p_lock_token text)
returns void language sql security definer set search_path = public as $function$
  update public.service_chats set payout_release_lock_token = null, payout_release_locked_at = null
  where id = p_chat_id and payout_release_lock_token = p_lock_token;
$function$;

create or replace function public.mark_service_payout_release_failed(p_chat_id uuid, p_lock_token text, p_reason text)
returns jsonb language plpgsql security definer set search_path = public as $function$
declare v_row public.service_chats%rowtype;
begin
  update public.service_chats set payout_release_failed_at = now(),
    payout_release_failure_reason = left(coalesce(nullif(btrim(p_reason), ''), 'unknown'), 500),
    payout_release_lock_token = null, payout_release_locked_at = null
  where id = p_chat_id and payout_release_lock_token = p_lock_token
    and payout_released_at is null and stripe_transfer_id is null returning * into v_row;
  return case when found then jsonb_build_object('updated', true, 'id', v_row.id, 'failure_reason', v_row.payout_release_failure_reason)
    else jsonb_build_object('updated', false) end;
end;
$function$;

create or replace function public.mark_service_payout_manual_recovery(p_chat_id uuid, p_lock_token text, p_reason text, p_stripe_transfer_id text default null)
returns jsonb language plpgsql security definer set search_path = public as $function$
declare v_row public.service_chats%rowtype;
begin
  update public.service_chats set manual_recovery_required_at = now(),
    manual_recovery_reason = left(coalesce(nullif(btrim(p_reason), ''), 'service_payout_manual_recovery_required'), 500),
    stripe_transfer_id = coalesce(nullif(btrim(coalesce(p_stripe_transfer_id, '')), ''), stripe_transfer_id),
    payout_release_failed_at = now(), payout_release_failure_reason = left(coalesce(nullif(btrim(p_reason), ''), 'service_payout_manual_recovery_required'), 500),
    payout_release_lock_token = null, payout_release_locked_at = null
  where id = p_chat_id and (payout_release_lock_token = p_lock_token or nullif(btrim(coalesce(p_lock_token, '')), '') is null)
    and payout_released_at is null returning * into v_row;
  return case when found then jsonb_build_object('updated', true, 'id', v_row.id, 'manual_recovery_required_at', v_row.manual_recovery_required_at, 'stripe_transfer_id', v_row.stripe_transfer_id)
    else jsonb_build_object('updated', false) end;
end;
$function$;

create or replace function public.mark_service_payout_released(p_chat_id uuid, p_lock_token text, p_stripe_transfer_id text)
returns jsonb language plpgsql security definer set search_path = public as $function$
declare v_row public.service_chats%rowtype;
begin
  if nullif(btrim(coalesce(p_stripe_transfer_id, '')), '') is null then raise exception 'stripe_transfer_id_required'; end if;
  update public.service_chats set payout_released_at = now(), stripe_transfer_id = p_stripe_transfer_id,
    payout_release_lock_token = null, payout_release_locked_at = null,
    payout_release_failed_at = null, payout_release_failure_reason = null
  where id = p_chat_id and payout_release_lock_token = p_lock_token
    and payout_released_at is null and manual_recovery_required_at is null returning * into v_row;
  return case when found then jsonb_build_object('updated', true, 'id', v_row.id, 'payout_released_at', v_row.payout_released_at, 'stripe_transfer_id', v_row.stripe_transfer_id)
    else jsonb_build_object('updated', false) end;
end;
$function$;
...
declare
  ...
  v_supabase_url text := nullif(btrim(current_setting('app.settings.supabase_url', true)), '');
  v_service_role_key text := nullif(btrim(current_setting('app.settings.service_role_key', true)), '');
begin
  for rec in
...
revoke all on function public.claim_service_payout_release(uuid, text) from public, anon, authenticated;
revoke all on function public.unlock_service_payout_release(uuid, text) from public, anon, authenticated;
revoke all on function public.mark_service_payout_release_failed(uuid, text, text) from public, anon, authenticated;
revoke all on function public.mark_service_payout_manual_recovery(uuid, text, text, text) from public, anon, authenticated;
revoke all on function public.mark_service_payout_released(uuid, text, text) from public, anon, authenticated;
revoke all on function public.process_service_payout_releases() from public, anon, authenticated;
grant execute on function public.claim_service_payout_release(uuid, text) to service_role;
grant execute on function public.unlock_service_payout_release(uuid, text) to service_role;
grant execute on function public.mark_service_payout_release_failed(uuid, text, text) to service_role;
grant execute on function public.mark_service_payout_manual_recovery(uuid, text, text, text) to service_role;
grant execute on function public.mark_service_payout_released(uuid, text, text) to service_role;
