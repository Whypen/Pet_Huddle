...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. spatial_ref_sys — PostGIS reference table; enable RLS + public read
--    PostGIS internal functions run as superuser and bypass RLS.
--    Enabling RLS without a policy would block app-level reads of the reference
--    data; add a permissive SELECT policy so ST_* functions that use this table
--    via PostgREST still work.
-- ─────────────────────────────────────────────────────────────────────────────
