-- Recreate RPC with strict profile display_name guard.
...
begin
  ...
  values (v_uid, v_display_name, v_legal_name, now())
  ...
  if v_post_to_threads then
    insert into public.threads (
      ...
      v_uid,
      format('%s Alert: %s', v_type, coalesce(v_address, 'Pinned Location')),
