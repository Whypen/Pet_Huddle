...
create table if not exists public.user_safety_metric_resets (
  user_id uuid primary key references auth.users(id) on delete cascade,
  penalty_count_offset bigint not null default 0,
  penalty_score_offset bigint not null default 0,
  false_report_count_offset bigint not null default 0,
  note text null,
  updated_by uuid not null references auth.users(id) on delete restrict,
  updated_at timestamptz not null default now()
);

revoke all on table public.user_safety_metric_resets from public;
revoke all on table public.user_safety_metric_resets from anon;
revoke all on table public.user_safety_metric_resets from authenticated;
revoke all on table public.user_safety_metric_resets from service_role;
grant select, insert, update, delete on table public.user_safety_metric_resets to service_role;

create or replace function public.admin_reset_user_safety_record(
...
declare
  ...
  v_penalty_count bigint := 0;
  v_penalty_score bigint := 0;
  v_false_report_count bigint := 0;
...
begin
  ...
  into v_penalty_count, v_penalty_score
  from public.admin_audit_logs aal
  where aal.target_user_id = p_target_user_id
    and aal.action in ('reports_warn', 'reports_shadow_restrict', 'reports_hard_ban');

  select coalesce(rfp.false_report_count, 0)
  into v_false_report_count
  from public.reporter_false_report_penalties rfp
  where rfp.reporter_user_id = p_target_user_id;

  insert into public.user_safety_metric_resets (
    user_id,
    penalty_count_offset,
    penalty_score_offset,
    false_report_count_offset,
    note,
    updated_by,
    updated_at
  )
  ...
  on conflict (user_id) do update
    set penalty_count_offset = excluded.penalty_count_offset,
        penalty_score_offset = excluded.penalty_score_offset,
        false_report_count_offset = excluded.false_report_count_offset,
        note = excluded.note,
  ...
  values (
    ...
    jsonb_build_object(
      'source', 'manual',
      'penalty_count_offset', v_penalty_count,
      'penalty_score_offset', v_penalty_score,
      'false_report_count_offset', v_false_report_count
    )
  );
...
revoke all on function public.admin_reset_user_safety_record(uuid, text) from public;
revoke all on function public.admin_reset_user_safety_record(uuid, text) from anon;
revoke all on function public.admin_reset_user_safety_record(uuid, text) from authenticated;
revoke all on function public.admin_reset_user_safety_record(uuid, text) from service_role;
grant execute on function public.admin_reset_user_safety_record(uuid, text) to authenticated;
grant execute on function public.admin_reset_user_safety_record(uuid, text) to service_role;
...
reset_offsets as (
  select
    u.user_id,
    coalesce(r.penalty_count_offset, 0)::bigint as penalty_count_offset,
    coalesce(r.penalty_score_offset, 0)::bigint as penalty_score_offset,
    coalesce(r.false_report_count_offset, 0)::bigint as false_report_count_offset
  from (select distinct user_id from candidate_users) u
  left join public.user_safety_metric_resets r on r.user_id = u.user_id
),
adjusted_penalties as (
  select
    ap.user_id,
    greatest(ap.penalty_count - ro.penalty_count_offset, 0)::bigint as penalty_count,
    greatest(ap.cumulative_penalty_score - ro.penalty_score_offset, 0)::bigint as cumulative_penalty_score,
    ap.latest_penalty_at
  from audit_penalties ap
  left join reset_offsets ro on ro.user_id = ap.user_id
),
adjusted_false_reports as (
  select
    fr.user_id,
    greatest(fr.false_report_count::bigint - ro.false_report_count_offset, 0)::int as false_report_count,
    fr.last_penalized_at
  from false_reports fr
  left join reset_offsets ro on ro.user_id = fr.user_id
),
moderation_state as (
  select
    u.user_id,
...
trust_model as (
  ...
  left join adjusted_penalties ap on ap.user_id = pn.user_id
  left join adjusted_false_reports fr on fr.user_id = pn.user_id
)
select
  pn.user_id,
  ...
  rc.reports_received,
  rc.reports_filed,
  afr.false_report_count,
  (ap.penalty_count + case when afr.false_report_count > 0 then 1 else 0 end)::bigint as penalty_count,
  (ap.cumulative_penalty_score + (afr.false_report_count * 2))::bigint as cumulative_penalty_score,
  tm.trust_score,
  ...
  greatest(
    ...
    coalesce(afr.last_penalized_at, to_timestamp(0)),
...
left join adjusted_penalties ap on ap.user_id = pn.user_id
left join adjusted_false_reports afr on afr.user_id = pn.user_id
