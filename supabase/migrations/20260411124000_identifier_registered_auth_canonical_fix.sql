  ...
  if v_email is not null then
    select exists (
      ...
      where lower(coalesce(u.email, '')) = v_email
        and (p.id is null or p.account_status <> 'removed')
    ) into v_email_exists;
  end if;
...
is 'Checks if email/phone is already occupied at auth identity level (or active profile) and returns moderation block/review signals.';
