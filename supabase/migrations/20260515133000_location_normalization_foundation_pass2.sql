-- Pass 2: normalized incident location foundation.
-- Stores reverse-geocoded alert location components when the app can provide them.

alter table public.map_alerts
  add column if not exists incident_country_code text,
  add column if not exists incident_country_name text,
  add column if not exists incident_admin_area text,
  add column if not exists incident_city text,
  add column if not exists incident_district text,
  add column if not exists incident_location_source text,
  add column if not exists incident_location_confidence text;
...
declare
  ...
  v_incident_country_code text := upper(nullif(btrim(payload->>'incident_country_code'), ''));
  v_incident_country_name text := nullif(btrim(payload->>'incident_country_name'), '');
  v_incident_admin_area text := nullif(btrim(payload->>'incident_admin_area'), '');
  v_incident_city text := nullif(btrim(payload->>'incident_city'), '');
  v_incident_district text := nullif(btrim(payload->>'incident_district'), '');
  v_incident_location_source text := nullif(btrim(payload->>'incident_location_source'), '');
  v_incident_location_confidence text := nullif(btrim(payload->>'incident_location_confidence'), '');
begin
  ...
  insert into public.broadcast_alerts (
    ...
    incident_country_name,
    incident_admin_area,
    incident_city,
    incident_district,
    incident_location_source,
    incident_location_confidence
  ) values (
    v_uid,
    v_type,
    v_title,
    ...
    v_thread_id,
    v_is_sensitive,
    v_incident_country_code,
    v_incident_country_name,
    v_incident_admin_area,
    v_incident_city,
    v_incident_district,
    coalesce(v_incident_location_source, 'alert'),
    v_incident_location_confidence
  )
...
revoke all on function public.create_alert_thread_and_pin(jsonb) from public, anon;
