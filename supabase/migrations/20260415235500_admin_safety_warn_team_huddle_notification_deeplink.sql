...
declare
  ...
  v_warn_notification_body text := 'You have received a message from Team Huddle. Tap to view';
  v_warn_notification_href text := null;
  v_warn_notification_dedupe_key text := null;
  v_warn_notification_id text := null;
begin
  ...
  if v_action = 'clear_restrictions' then
    ...
    v_warn_notification_href := '/chat-dialogue?room=' || v_warn_chat_id::text || '&with=' || v_warn_sender_user_id::text;
    v_warn_notification_dedupe_key := 'reports_warn:' || p_target_user_id::text || ':' || v_warn_chat_id::text || ':' || md5(coalesce(v_warn_message_body, ''));
    ...
    if not exists (
      ...
      where n.user_id = p_target_user_id
        and n.type = 'chats'
        and coalesce(n.data->>'warn_dedupe_key', '') = v_warn_notification_dedupe_key
    ) then
      select public.enqueue_notification(
        p_user_id := p_target_user_id,
        p_category := 'chats',
        p_kind := 'team_huddle_warn',
        p_title := 'Team Huddle',
        p_body := v_warn_notification_body,
        p_href := v_warn_notification_href,
        p_data := jsonb_build_object(
          'room_id', v_warn_chat_id,
          'with_user_id', v_warn_sender_user_id,
          'warn_dedupe_key', v_warn_notification_dedupe_key,
          'warn_message_sender_user_id', v_warn_sender_user_id,
          'warn_message_chat_id', v_warn_chat_id,
          'source', 'admin_safety_warn'
        )
      ) into v_warn_notification_id;
    end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'warn_message_sender_user_id', v_warn_sender_user_id,
      'warn_message_chat_id', v_warn_chat_id,
      'warn_notification_body', v_warn_notification_body,
      'warn_notification_href', v_warn_notification_href,
      'warn_notification_id', v_warn_notification_id,
      'warn_notification_dedupe_key', v_warn_notification_dedupe_key,
