...
as $$
  ...
  scoped_rooms as (
    ...
    where case coalesce(p_scope, 'all')
      ...
      when 'service' then c.type = 'service' or sc.chat_id is not null
      else true
    end
  ),
  last_messages as (
...
create or replace function public.get_chat_invariant_diagnostics()
returns table(
  issue text,
  chat_id uuid,
  details jsonb
...
as $$
  with room_members as (
    select c.id as chat_id, c.type as room_type, count(distinct crm.user_id)::int as member_count
    from public.chats c
    left join public.chat_room_members crm on crm.chat_id = c.id
  ...
  duplicate_pairs as (
    select
      dp.user_low,
      dp.user_high,
      array_agg(dp.chat_id order by dp.chat_id) as chat_ids,
      count(*) as pair_count
    from direct_pairs dp
    group by dp.user_low, dp.user_high
    having count(*) > 1
  )
  select
    'duplicate_direct_pair'::text as issue,
    unnest(dp.chat_ids) as chat_id,
    jsonb_build_object(
      'user_low', dp.user_low,
      'user_high', dp.user_high,
      'chat_ids', dp.chat_ids,
      'pair_count', dp.pair_count
    ) as details
  ...
  select
    'direct_member_count_invalid'::text as issue,
    rm.chat_id,
    jsonb_build_object(
  ...
  select
    'group_room_shape_invalid'::text as issue,
    rm.chat_id,
    jsonb_build_object(
      'room_type', rm.room_type,
      'member_count', rm.member_count,
      'has_service_row', exists(select 1 from public.service_chats sc where sc.chat_id = rm.chat_id)
    ) as details
  from room_members rm
  where rm.room_type = 'group'
    and (
      rm.member_count < 1 or
      exists(select 1 from public.service_chats sc where sc.chat_id = rm.chat_id)
    )

  union all

  select
    'service_room_shape_invalid'::text as issue,
    rm.chat_id,
    jsonb_build_object(
      'room_type', rm.room_type,
      'member_count', rm.member_count,
  ...
  from room_members rm
  where
    (rm.room_type = 'service' and (rm.member_count <> 2 or not exists(select 1 from public.service_chats sc where sc.chat_id = rm.chat_id)))
    or
    (rm.room_type <> 'service' and exists(select 1 from public.service_chats sc where sc.chat_id = rm.chat_id));
$$;

grant execute on function public.get_chat_invariant_diagnostics() to authenticated;
