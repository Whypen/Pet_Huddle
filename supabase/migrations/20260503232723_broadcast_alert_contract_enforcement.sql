-- Enforce active concurrent broadcast limits on direct broadcast_alerts inserts as well as RPC path.
...
drop trigger if exists trg_broadcast_alerts_contract on public.broadcast_alerts;
create trigger trg_broadcast_alerts_contract
before insert on public.broadcast_alerts
for each row
execute function public.enforce_broadcast_alert_contract();
