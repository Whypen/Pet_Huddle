-- Phase Monetization: Top Profile Booster entitlement (grant + expiry + discover ranking effect)

alter table public.profiles
  add column if not exists top_profile_boost_until timestamptz;

create index if not exists idx_profiles_top_profile_boost_until
  on public.profiles(top_profile_boost_until)
  where top_profile_boost_until is not null;
...
scored as (
  select
    ...
    (
      case
        when fg.top_profile_boost_until is not null and fg.top_profile_boost_until > now()
          then greatest(15, fg.tier_boost_score * 3)
        else 0
      end
    )::numeric as top_profile_boost_component,
