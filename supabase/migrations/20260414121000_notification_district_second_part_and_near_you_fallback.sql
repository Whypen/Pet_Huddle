-- Notification district contract:
-- - prefer exact alert.location_district
-- - otherwise extract district token from location_name
-- - fallback copy must render "near you"
...
begin
  ...
  if v_len >= 2 then
    ...
    -- Typical shapes:
    -- 1) "Street, District, Hong Kong" => second part is district
    -- 2) "District, Hong Kong" => first part is district
    if v_len = 2 and v_last in ('hong kong', 'hk', 'hksar', 'china', 'cn') then
      return nullif(btrim(coalesce(v_parts[1], '')), '');
    end if;
