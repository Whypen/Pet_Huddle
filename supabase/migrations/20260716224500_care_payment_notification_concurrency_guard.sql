...
-- Stripe webhooks and the recovery worker may observe the same transition at
-- the same time. Serialize a notification subject inside the transaction so
-- enqueue_notification's existing subject-key check remains single-fire even
-- under concurrent delivery.
create or replace function public.service_notify_once_serialized(
  p_user_id uuid,
  p_kind text,
  p_title text,
  p_body text,
...
declare
  v_lock_key text;
begin
  v_lock_key := p_user_id::text || ':' || lower(btrim(coalesce(p_kind, ''))) || ':'
    || coalesce(p_data->>'notification_key', p_href, '');
  perform pg_advisory_xact_lock(hashtextextended(v_lock_key, 0));
  return public.service_notify(p_user_id, p_kind, p_title, p_body, p_href, p_data);
end;
$function$;

revoke all on function public.service_notify_once_serialized(uuid, text, text, text, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.service_notify_once_serialized(uuid, text, text, text, text, jsonb)
  to service_role;
