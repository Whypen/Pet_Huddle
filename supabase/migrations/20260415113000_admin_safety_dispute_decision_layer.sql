-- /admin/safety disputes decision layer (admin-only, no Stripe execution)
-- Scope: persist decision payload/status + audit trail only.
...
alter table public.service_disputes
  ...
  add column if not exists decision_actor_id uuid references public.profiles(id) on delete set null,
...
begin
  ...
  if v_action not in ('hold_funds', 'release_full', 'partial_refund', 'full_refund') then
  ...
  if v_action = 'hold_funds' then
    v_status := 'resolved_hold';
    v_refund := 0;
    v_provider := 0;
    v_audit_action := 'disputes_hold_funds';
  elsif v_action = 'release_full' then
  ...
  v_money := jsonb_build_object(
    ...
    'currency', coalesce(v_chat.quote_card->>'currency', 'usd')
  );
