...
const SCRIPT_ID = "cf-turnstile-health-script";
...
const TurnstileHealth = () => {
  ...
  useEffect(() => {
    window.__huddleTurnstileHealth = {
      ...
      .then(() => {
        ...
        const nextWidgetId = window.turnstile.render(containerRef.current, {
          ...
          callback: (token) => {
            ...
            if (window.__huddleTurnstileHealth) {
              window.__huddleTurnstileHealth.callbackFired = true;
              window.__huddleTurnstileHealth.tokenLength = nextLength;
            }
          },
          "error-callback": () => {
            setErrorCallbackFired(true);
            if (window.__huddleTurnstileHealth) {
              window.__huddleTurnstileHealth.errorCallbackFired = true;
            }
          },
          "expired-callback": () => {
            setExpiredCallbackFired(true);
            if (window.__huddleTurnstileHealth) {
              window.__huddleTurnstileHealth.expiredCallbackFired = true;
            }
          },
        });
        ...
        if (window.__huddleTurnstileHealth) {
          window.__huddleTurnstileHealth.widgetRendered = true;
          window.__huddleTurnstileHealth.widgetId = nextWidgetId;
        }
      })
      .catch(() => {
    ...
    <div className="min-h-svh bg-background px-6 py-10 text-brandText">
      <div className="mx-auto max-w-md space-y-6 rounded-3xl bg-white/90 p-6 shadow-[0_20px_60px_rgba(70,90,130,0.18)]">
        <div className="space-y-2">
          <h1 className="text-2xl font-semibold">Turnstile Health</h1>
          <p className="text-sm text-muted-foreground">Minimal live widget probe using the current production sitekey.</p>
        </div>
...
export default TurnstileHealth;
