...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    if (savedIdentifier) {
      setValue("email", savedIdentifier, { shouldValidate: true });
    }
  }, [setValue]);
  ...
  const oauthEnabled = (provider: "google" | "apple") => {
    if (provider === "google") return import.meta.env.VITE_OAUTH_GOOGLE_ENABLED === "true";
    ...
    <div className="min-h-screen bg-gradient-to-b from-primary-soft via-background to-accent-soft flex flex-col px-6">
      ...
      <div className="bg-card rounded-3xl shadow-elevated p-6 max-w-md mx-auto w-full">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          {authError && (
            <Button type="button" variant="outline" className="w-full h-10" onClick={() => handleCreateAccount(watch("email") || "")}>
              Create account
            </Button>
          )}
        </form>
        ...
        <div className="space-y-2">
          <div
            className={!oauthEnabled("apple") ? "cursor-not-allowed opacity-60" : ""}
            onClick={() => {
              if (!oauthEnabled("apple")) toast.error("Sign-in not configured");
            }}
          >
            <Button
              type="button"
          ...
          <div
            className={!oauthEnabled("google") ? "cursor-not-allowed opacity-60" : ""}
            onClick={() => {
              if (!oauthEnabled("google")) toast.error("Sign-in not configured");
            }}
          >
            <Button
              type="button"
        ...
        <div className="mt-4 text-center text-sm text-muted-foreground">
          ...
          <button
            ...
            onClick={() => handleCreateAccount(watch("email") || "")}
          >
            Sign up
          </button>
        </div>
