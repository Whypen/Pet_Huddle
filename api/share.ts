...
const normalizeSocialId = (value: string) => value.trim().replace(/^@+/, "");
...
const buildTitle = (displayName: string, socialId: string) => {
...
const fetchThreadPreviewData = async (threadId: string) => {
  ...
  return {
    ...
    description: buildDescription(String(thread.content || "")),
  };
};
...
<html lang="en">
  ...
  <body>
    <main class="wrap">
      <article class="card">
        <div class="preview"><img src="${safeImage}" alt="huddle preview image" /></div>
