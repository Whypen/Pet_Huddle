...
export function MapScreen() {
  ...
  const reportFriend = useCallback(async () => {
    if (!user?.id || !selectedFriend?.id) return;
    if (selectedFriend.id === user.id) {
      Alert.alert("Report", "You cannot report your own account.");
      return;
    }

    try {
      const { data, error } = await supabase.rpc("process_user_report", {
        p_target_id: selectedFriend.id,
        p_categories: ["Other"],
        p_details: "Reported from native map friends surface.",
        p_attachment_urls: [],
      });
      if (error) throw error;

      const action = data && typeof data === "object" && "action" in data ? String(data.action || "") : "";
      Alert.alert(
        action === "evidence_appended" ? "Report updated" : "Reported",
        "Thanks for helping keep Huddle safe. Our team will review this report."
      );
      ...
      Alert.alert("Report failed", "We couldn't submit this report. Please try again or contact support.");
    }
  ...
  const blockFriend = useCallback(async () => {
    if (!user?.id || !selectedFriend?.id) return;
    if (selectedFriend.id === user.id) {
      Alert.alert("Block", "You cannot block your own account.");
      return;
    }
    ...
    try {
      const { error } = await supabase.from("user_blocks").insert({
        blocker_id: user.id,
        blocked_id: selectedFriend.id,
      });
      if (error) throw error;

      setFriends((current) => current.filter((friend) => friend.id !== selectedFriend.id));
      setSelectedFriend(null);
      Alert.alert("User blocked", "You will no longer see this user in launch-scope native surfaces.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "";
      ...
      Alert.alert("Block failed", "We couldn't block this user. Please try again or contact support.");
    }
  }, [selectedFriend?.id, user?.id]);
      ...
      <Modal visible={!!selectedFriend} transparent animationType="slide" onRequestClose={() => setSelectedFriend(null)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "flex-end" }}>
          <View style={{ backgroundColor: COLORS.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 16, maxHeight: "70%" }}>
            ...
            <Pressable
              onPress={() => {
                Alert.alert("Block user", "You won't see this user in launch-scope native surfaces after blocking.", [
              ...
              style={({ pressed }) => ({
                ...
                borderRadius: 12,
                borderWidth: 1,
                borderColor: `${COLORS.brandError}40`,
                paddingVertical: 12,
                alignItems: "center",
                opacity: pressed ? 0.9 : 1,
              })}
            >
              <HText variant="body" style={{ fontWeight: "900", color: COLORS.brandError }}>
                Block user
              </HText>
            </Pressable>
            <Pressable
