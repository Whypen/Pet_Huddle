...
### 2.2 PII Auto‑Delete
- **Cron Job:** `pii_purge_daily` runs daily, deletes identity_verification images 7 days after verification status becomes `approved` or `rejected`

### 2.3 PostGIS & Radius Search
- **Radius Range:** 0‑150km
- **Index:** GIST index on `profiles.location` (geography)
- **Expansion:** UI button expands radius +15km, max 150km, resets hidden list
...
### 3.1 Gemini AI Vet
- **Routing:** Gemini 1.5 Flash for text, Gemini 1.5 Pro for image
...
- **Client Idempotency Key:** generated client‑side and passed to Edge Function header
...
- Expand Search: adds +15km, resets local hidden list
...
- Inline quote format: `> @user: "First 50 chars..."`
...
### 5.1 Red Error Rule
All validation failures must show **red error text below the field** and block submit.

### 5.2 Grey Subtext Rule
Vaccination inputs must show: **"Input last vaccination dates for better tracking"** in pt12 grey.

### 5.3 Admin Hatch
Long‑press version number reveals **Admin Control Center** (`/admin/control-center`).
...
- Security: RLS admin update policy enforced

---

## 7. Release Verification (Minimum Bar)
