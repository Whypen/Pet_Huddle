...
import { resolveNativeViewerScope, type NativeViewerScopePoint } from "../lib/nativeViewerScope";
...
export function NativeMapScreen({
  ...
  }, [broadcastOpen, hasFullAlertDetail, mapRestrictionModalOpen, onBottomSheetOpenChange, profileModalUserId, selectedAlert, showGpsModal, showUnpinConfirm]);
  ...
  const mapSessionCacheKey = useMemo(() => (effectiveUserId ? `huddle:native-map-session:v5:${mapShellSessionKey}` : null), [effectiveUserId, mapShellSessionKey]);
  ...
  const shellToFriendMarker = useCallback((shell: NativeMapPinShell): NativeMapFriendPin => ({
    id: shell.pin_id,
    display_name: shell.display_name,
    avatar_url: shell.avatar_url,
    is_verified: shell.is_verified,
    is_invisible: shell.is_invisible,
    gender_genre: shell.gender_genre,
    last_lat: shell.lat,
    last_lng: shell.lng,
    location_pinned_until: shell.updated_at,
    marker_state: "active",
  }), []);

  const loadMapData = useCallback(async (options?: { center?: [number, number] | null; force?: boolean; useCameraCenter?: boolean }) => {
    ...
    if (!options?.force && mapShellRequestKeyRef.current === viewportBucketKey) return;
    ...
    const run = async () => {
      ...
      const [viewerScope, nextOwnPin] = await Promise.all([
        resolveNativeViewerScope({ userId: effectiveUserId, accessToken }),
        fetchNativeMapOwnPin(effectiveUserId, { accessToken }),
      ]);
      ...
      const shellGroups = await Promise.all(anchors.map((anchor) =>
        fetchVisibleMapPinShells(anchor.center, anchor.radiusMeters, {
          accessToken,
      ...
      const friendShells = shells.filter((shell) => !shell.is_alert).map(shellToFriendMarker);

      setAlerts(sortNativeMapAlertsForDisplay(alertShells, center));
      setFriends(friendShells);
      setOwnPin(nextOwnPin);
      lastFetchedMapAreaRef.current = { center, zoomBucket };
      applyCamera(center, cameraZoomRef.current, false);
  ...
  }, [accessToken, applyCamera, effectiveUserId, mapSessionCacheKey, mapShellSessionKey, shellToAlertMarker, shellToFriendMarker]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (mapSessionCacheKey) {
        try {
          const raw = await AsyncStorage.getItem(mapSessionCacheKey);
          ...
          if (
            ...
            setOwnPin(cached.ownPin ?? null);
            applyCamera(cached.center, typeof cached.cameraZoom === "number" ? cached.cameraZoom : cameraZoomRef.current, false);
      ...
      if (!initialCenter && effectiveUserId) {
        const ownPin = await fetchNativeMapOwnPin(effectiveUserId, { accessToken }).catch(() => null);
        if (active && ownPin) initialCenter = [ownPin.lng, ownPin.lat];
      }

      await loadMapDataRef.current(initialCenter ? { center: initialCenter } : undefined);
    })()
      .catch(() => {
        if (!active) return;
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [accessToken, alertFocus, alertFocusRetries, alerts, applyCamera, effectiveUserId, mapShellSessionKey]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await pinNativeUserLocation(effectiveUserId, coords.lat, coords.lng, address, { accessToken });
      setDeviceLocation(coords);
  ...
  const requestPinFromLiveGps = async () => {
    ...
    if (!accessToken) {
      setStatusMessage("Please login again to pin location");
      if (__DEV__) console.log("NATIVE_MAP_SELF_PIN_RESULT", { ok: false, reason: "missing_access_token" });
      return;
    }
    if (permissionState === "denied" && !permissionDetail.canAskAgain) {
      setShowGpsModal(true);
      if (__DEV__) console.log("NATIVE_MAP_SELF_PIN_RESULT", { ok: false, reason: "permission_denied" });
      return;
    }
    setPinning(true);
    setStatusMessage(null);
    try {
      ...
      if (nextPermission.state !== "granted") {
        if (!nextPermission.canAskAgain) setShowGpsModal(true);
        if (__DEV__) console.log("NATIVE_MAP_SELF_PIN_RESULT", { ok: false, reason: "permission_not_granted", state: nextPermission.state });
        return;
      }
      const coords = await getNativeCurrentCoordinates({ accuracy: "high", force: true });
      if (!coords) {
        setShowGpsModal(true);
        if (__DEV__) console.log("NATIVE_MAP_SELF_PIN_RESULT", { ok: false, reason: "missing_coordinates" });
        return;
      }
      if (__DEV__) {
        console.log("NATIVE_MAP_SELF_PIN_START", {
      ...
      const address = await lookupNativeMapAddress(coords.lat, coords.lng);
      await pinNativeUserLocation(effectiveUserId, coords.lat, coords.lng, address, { accessToken });
      const profileSnapshot = await fetchNativeProfileSummary(effectiveUserId, { force: false, accessToken }).catch(() => null);
      ...
      console.warn("[native.map] pin_location_failed", error);
      setStatusMessage(error instanceof Error && error.message === "missing_access_token" ? "Please login again to pin location" : "Failed to pin location");
      if (__DEV__) {
  ...
  const toggleInvisible = async () => {
    ...
    try {
      await setNativeMapInvisible(effectiveUserId, nextInvisible, { accessToken });
    } catch {
      setOwnPin({ ...ownPin, is_invisible: !nextInvisible });
  ...
  return (
    <View style={styles.container}>
      <Mapbox.MapView
        ...
        onMapIdle={() => {
          ...
          cameraIdleFetchTimerRef.current = setTimeout(() => {
            ...
            if (previous && movedKm < MAP_CAMERA_REFETCH_MIN_KM && previous.zoomBucket === nextZoomBucket) return;
          ...
          >
            <Pressable
              ...
              onPress={() => {
                ...
                setSelectedAlert(alert);
                if (__DEV__) {
                  console.log("NATIVE_MAP_ALERT_TAP", {
                    alertId: alert.id,
                    hasAccessToken: Boolean(accessToken),
                ...
                const requestSessionKey = mapShellSessionKey;
                const cacheKey = `huddle:native-map-pin-detail:v6:${effectiveUserId || "anon"}:${requestSessionKey}:${alert.id}:${alert.created_at}`;
                void (async () => {
                  try {
                    if (sessionKeyRef.current !== requestSessionKey) return;
                    const cachedRaw = await AsyncStorage.getItem(cacheKey);
                    if (cachedRaw) {
                      const parsed = JSON.parse(cachedRaw) as NativeMapAlert | { value?: NativeMapAlert | null };
                      const cached = (parsed && "value" in parsed && parsed.value ? parsed.value : parsed) as NativeMapAlert;
                      const hasRealDetail = Boolean(
                        cached.title ||
                        cached.description ||
                        cached.media_urls?.length ||
                        cached.photo_url ||
                        cached.creator?.display_name ||
                        cached.creator?.avatar_url
                      );
                      if (hasRealDetail) {
                        if (__DEV__) console.log("NATIVE_MAP_PIN_DETAIL_CACHE_HIT", {
                          pin_id: alert.id,
                          updated_at: alert.created_at,
                          title: cached.title,
                          description: cached.description,
                          creator: cached.creator?.display_name,
                          avatar: cached.creator?.avatar_url,
                        });
                        if (sessionKeyRef.current !== requestSessionKey) return;
                  ...
                  try {
                    if (__DEV__) {
                      console.log("NATIVE_MAP_ALERT_DETAIL_FETCH", {
                        alertId: alert.id,
                        hasAccessToken: Boolean(accessToken),
                        source: "alert",
                      });
                    }
                    const detail = effectiveUserId && accessToken ? await fetchNativeMapAlertById(alert.id, effectiveUserId, {
                      accessToken,
                      sessionKey: requestSessionKey,
                      source: "alert",
                      updatedAt: alert.created_at,
                    ...
                    if (sessionKeyRef.current !== requestSessionKey) return;
                    if (detail) {
                      if (__DEV__) console.log("NATIVE_MAP_PIN_DETAIL_FETCHED", {
                        pin_id: alert.id,
                        updated_at: alert.created_at,
                        title: detail.title,
                        description: detail.description,
                        creator: detail.creator?.display_name,
                      ...
                      console.warn("NATIVE_MAP_PIN_DETAIL_MISSING", { pin_id: alert.id, updated_at: alert.created_at });
                      setStatusMessage(accessToken ? "Unable to open alert details. Tap the pin to retry." : "Please login again to open alert details.");
                    }
                  } catch (error) {
                    console.warn("NATIVE_MAP_PIN_DETAIL_FAILED", {
                      pin_id: alert.id,
                      error: error instanceof Error ? error.message : String(error),
                    });
                    setStatusMessage(error instanceof Error && error.message === "missing_access_token" ? "Please login again to open alert details." : "Unable to open alert details. Tap the pin to retry.");
                  }
                })();
        ...
        onPrimary={async () => {
          if (permissionDetail.canAskAgain) {
            ...
            if (detail.state === "granted") {
              ...
              const coords = await getNativeCurrentCoordinates({ accuracy: "high", force: true }).catch(() => null);
              setDeviceLocation(coords);
              if (coords) applyCamera([coords.lng, coords.lat], 15.5);
      ...
      <NativeAlertDetailModal
        ...
        onOpenProfile={(profileId) => { setSelectedAlert(null); setProfileModalUserId(profileId); }}
